...
asian_1_field = dbc.Row([
    dbc.Col(html.H6('Торговых дней в периоде')),
    dbc.Col(dcc.Input(id='avg_periods', value=2, type='number', style=input_style))
    ], style = row_style)

asian_1_5_field = dbc.Row([
    dbc.Col(html.H6('Календарных дней в периоде')),
    dbc.Col(dcc.Input(id='calend_periods', value=2, type='number', style=input_style))
    ], style = row_style)
...
asian_inputs = html.Div(children=[asian_1_field, asian_1_5_field, asian_2_field, averaging_field], style={'width':'450px',
