...
@app.callback(Output("avg_periods", "value"), Output("calend_periods", "value"), Input("date_field", 'end_date'))
def maintain_periods(end_date):
    return floor((pd.to_datetime(end_date, dayfirst=True)-pd.to_datetime(datetime.date.today().strftime("%Y-%m-%d"),
           ...
           (pd.to_datetime(end_date, dayfirst=True)-pd.to_datetime(datetime.date.today().strftime("%Y-%m-%d"), dayfirst=True)).days
